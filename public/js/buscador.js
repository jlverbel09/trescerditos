    ...
    $.fn.searchit = function(options) {
      ...
      return this.each(function() {
        ...
        if (opts.textField == null) {
          $t.before("<input type='textbox' autofocus class='form-control form-control-sm dark' placeholder='Producto' id='__searchit" + $counter + "'>");
        ...
        //MY CODE-------------------------------------------------------------------
        if (opts.selected) opts.textField.val($(this).find(":selected").val().trim());
        //MY CODE ENDS HERE -------------------------------------------------------
        if (opts.dropDown) {
          $t.css("padding", "5px")
        ...
        $t.click(function(event) {
          _opts($t).textField.val($(this).find(":selected").text().trim());
          _opts($t).wrp.hide();
        ...
        _opts($t).textField.keyup(function(event) {
          if (event.keyCode == 13) {
            $(this).val($t.find(":selected").text().trim());
            _opts($t).wrp.hide();
            return;
          }
