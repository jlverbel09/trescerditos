...
class LogsController extends Controller
{
    public function index()
    {
        ...
        $aperturas = Log::select('logs.id_mesa', 'mesas.descripcion as nombremesa', 'ticket', 'logs.created_at')
            ->addSelect(DB::raw("(select updated_at  from logs lgs where lgs.ticket=logs.ticket and logs.id_mesa = lgs.id_mesa and accion = 'CIERRE MESA' order by updated_at desc limit 1) as cierre"))
